    ...
    def __init__(self, arch, sigma_sq):
        ...
        with tf.name_scope("action_mu"):
            ...
            with tf.name_scope("mu"):
                self.mu = tf.tanh(tf.matmul(a_2, W_3) + b_3)
...
def reinforce(pi, env, n_iter, n_eps, gamma):
    """ Execute REINFORCE to train the argument policy (pi) in the argument environment. """
    for ep in tqdm.trange(n_eps):
        episode = rollout(pi, env, n_iter)
        pi.learn(ep, episode.states, episode.returns(gamma))

def rollout(pi, env, n_iter, render=False):
...
def main(args):
    ...
    # Run REINFORCE.
    pi = GaussianPolicy(arch, args.sigma_sq)
    reinforce(pi, env, args.n_iter, args.n_eps, args.gamma)

    # Visually test the trained policy.
    input("Training complete, press ENTER when ready.")
    rollout(pi, env, args.n_iter, render=True)    
...
if __name__ == "__main__":
    ...
    parser.add_argument("--n_iter", type=int, default=10000, help="Maximum number of iterations")
    parser.add_argument("--gamma", type=float, default=0.99, help="Discount factor for rewards")
    parser.add_argument("--sigma_sq", type=float, default=0.01, help="Variance for exploration")
